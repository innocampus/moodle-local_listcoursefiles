...
class course_file {
     ...
     *
     * The record must have a property for every column in the `files` table, plus `contextlevel` and `instanceid` from
     * the joined `context` table. Known user name fields (per {@see user_fields::get_name_fields}) are peeled off into a
     * dedicated user object; any other unrecognized columns will trigger an `Unknown named parameter` error at the
     * constructor call. This is intentional — it surfaces a SQL/constructor drift loudly instead of silently absorbing
     * the column.
     *
     ...
     * @param stdClass $user Joined `user` name fields (empty stdClass if none were selected).
     */
